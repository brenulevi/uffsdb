...
{
  return (table_type == 'D' && insert_type == 'I') || (table_type == 'D' && insert_type == 'D') || (table_type == 'I' && insert_type == 'I') || (table_type == 'S' && insert_type == 'S') || (table_type == 'S' && insert_type == 'C') || (table_type == 'C' && insert_type == 'C') || (table_type == 'C' && insert_type == 'S');
}
////
...
int verificaChaveFK(char *nomeTabela, column *c, char *nomeCampo, char *valorCampo, char *tabelaApt, char *attApt)
{
  ...
  for (page = 0; page < PAGES; page++)
  {
    ...
    {
      if (strcmp(pagina[j].nomeCampo, "") != 0)
      {
...
int finalizaInsert(char *nome, column *c)
{
  ...
  for (j = 0, temp = c; j < objeto.qtdCampos && temp != NULL; j++, temp = temp->next)
  {
    ...
    case FK:
      ...
      free(arquivoIndice);
      if (raizfk == NULL)
      { // se não encontra faz a verificação sem indice b+
      ...
      { // atributo referenciado possui indice B+
  ...
  for (auxC = c, t = 0; auxC != NULL; auxC = auxC->next, t++)
  {
    ...
    else if (auxT[t].tipo == 'I')
    { // Grava um dado do tipo inteiro.
      i = 0;
    ...
    else if (auxT[t].tipo == 'D')
    { // Grava um dado do tipo double.
      x = 0;
    ...
    else if (auxT[t].tipo == 'C')
    { // Grava um dado do tipo char.
...
void printConsulta(Lista *p, Lista *l)
{
       ...
       j; j = j->prox, i = i->prox)
  {
    ...
    if (jw->id == (int)'S')
      printf(" %-20s ", (char *)(i->inf));
    else
      printf(" %-10s ", (char *)(i->inf));
    if (j->prox)
      printf("|");
  }
  printf("\n");
...
int excluirTabela(char *nomeTabela)
{
  ...
  for (i = 0; i < objeto.qtdCampos; i++)
  {
    ...
    {
      for (j = 0; j < qtTable; j++)
      { // se tiver chave primaria verifica se ela e chave
        if (objcmp(tupla[j], nomeTabela) != 0)
        { // estrangeira em outra tabela
          ...
          {
            if (tab3[l].chave == FK)
            { // verifica se a outra tabela possui chave estrangeira. se sim, verifica se e da tabela anterior.
...
void createTable(rc_insert *t)
{
  ...
  if (finalizaTabela(tab) == SUCCESS)
  {
    ...
    {
      if (t->attribute[i] == PK)
      { // procura o atributo PK e cria o arquivo de índice
  ...
  { // Tabela já existe, então não é preciso criar o índice b+.
    printf("ERROR: table already exist\n");
  }
...
void createIndex(rc_insert *t)
{
  ...
  {
    if (strcmp(aux->nome, t->columnName[0]) == 0)
    { // Procura o atributo na tabela
      if (aux->chave == PK)
      { // Se o atributo é PK já possui índice criado automaticamente
